...
- `room_scene_anchor`：识别房间大厅场景锚点，避免在其他界面误判。
- `room_area_change`：记录玩家列表区域变化，辅助发现异常遮挡或状态刷新。

战斗场景至少需要三个 HUD 锚点中的两个。确认进入战斗后会暂停所有 `ROOM` 元素；退出后必须连续取得 3 个有效且有人的槽位快照，房间 Overlay 才恢复。HP 或 SHIELD 色条证据不足时显示 `--`，不会误报为 `0%`。
...
本项目只监测屏幕像素和窗口区域，不读取游戏内存、不注入进程、不绕过反作弊。不同游戏对截图和覆盖层的兼容性不同，独占全屏模式不作为 v1 验收目标。
